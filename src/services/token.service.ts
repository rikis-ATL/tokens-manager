...
export class TokenService {
  ...
  generateStyleDictionaryOutput(groups: TokenGroup[], globalNamespace: string): any {
    ...
    const addTokenToOutput = (token: GeneratedToken, groupPath: string[]) => {
      ...
      // Add the token — omit auto-generated descriptions (e.g. "Generated by Json node")
      const isAutoDescription = (d: string) => /^Generated by .+ node/.test(d);
      current[token.path] = {
        $type: token.type,
        $value: token.value,
        ...(token.description && !isAutoDescription(token.description) && { $description: token.description }),
